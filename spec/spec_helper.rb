...
RSpec.configure do |c|
  ...
  if tags.include?(:live) || tags.include?(:oauth)
    ...
    if ENV['IS_TRAVIS']
      ...
      when '2.3.8'  then ENV['STRIPE_TEST_SECRET_KEY_A']
      when '2.4.6' then ENV['STRIPE_TEST_SECRET_KEY_B']
      when '2.5.5'  then ENV['STRIPE_TEST_SECRET_KEY_C']
      when '2.6.3'  then ENV['STRIPE_TEST_SECRET_KEY_D']
      end
    else
