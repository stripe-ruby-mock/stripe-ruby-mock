...
shared_examples 'Invoice API' do
  ...
  context "retrieving upcoming invoice" do
    ...
    let(:coupon_amtoff) { @teardown_coupon_amtoff = true; stripe_helper.create_coupon(id: '100OFF', currency: 'usd', amount_off: 100_00, duration: 'repeating', duration_in_months: 6) }
    let(:coupon_pctoff) { @teardown_coupon_pctoff = true; stripe_helper.create_coupon(id: '50%OFF', currency: 'usd', percent_off: 50, amount_off: nil, duration: 'repeating', duration_in_months: 6) }
    ...
    describe 'proration' do
      ...
      [false, true].each do |with_trial|
        describe "prorating a subscription with a new plan, with_trial: #{with_trial}" do
          let(:new_monthly_plan) { @teardown_monthly_plan = true; stripe_helper.create_plan(id: '100m', amount: 100_00, interval: 'month', name: '100m', currency: 'usd') }
          let(:new_yearly_plan) { @teardown_yearly_plan = true; stripe_helper.create_plan(id: '100y', amount: 100_00, interval: 'year', name: '100y', currency: 'usd') }
